...
class BrowserAutomation {
    ...
    async runAccount(account, proxy, config) {
        ...
        try {
            ...
            console.log(
                `[${account.username}] STEP 2: Creating browser context...`,
            );
    ...
    async handleFinalSteps(page, accountLabel) {
        ...
        const prenotaSpan = await page.waitForFunction(
            () =>
                [...document.querySelectorAll(".v-btn__content")].find(
                    (el) => el.textContent.trim().toLowerCase() === "prenota",
                ),
            { timeout: 60000 },
        );

        const prenotaBtn = await prenotaSpan.evaluate((span) =>
            span.closest("button"),
        );
        ...
        console.log(
            `[${accountLabel}] PRENOTA button found. Ready for final booking.`,
        );
    ...
    async realClick(page, element, label) {
        ...
        // Use Playwright's click method instead of page.evaluate
